 ...
 * pam_email.c
 *
 * Autores: Jeferson Ferreira da Cruz, Jhonatan Mello, Afonso Camargo, Isabella Bomfim, Thiago Yuji
 *
 * Parts taken from pam_email.c by Peter Surda <surda@economicsofbitcoin.com>
...
#define EMAIL_BODY "Requisição de autenticação "
#define EMAIL_SUBJ_PRE ""
#define TOKEN_MESSAGE "Token: "
#define KEY_SYMBOLS "123456789abcdefghijkmnopqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ!@#$%&"
#define KEY_LEN 12
#define EMAIL_SUBJ_POST " - requisição de autenticação"
...
/*
 * Struct é uma variavel especial que contem diversas outras variaveis normalmente de tipos diferentes e são denominadas membros da struct
 * Esse escopo é referente as informações do email
 */
...
/*
 * Facilita a obtenção de argumentos. Os argumentos aceitos são da forma: nome=valor
 *
 * @param argn- nome do argumento para obter
 * @param argc- número total de argumentos
 * @param argv- argumentos
 * @return Ponteiro para Valor ou NULL
 */

/*
 * Static é um modificador de armazenamento que pode ser utilizado para variáveis globais e locais, e tem como função indicar que tal variável é permanente
 *
 */
static const char* get_arg(const char* argn, int argc, const char** argv) {
	...
	for (i = 0; i < argc; i++) {
		if (strncmp(argn, argv[i], len) == 0 && argv[i][len] == '=') {
			// só da uma parte da URL (depois do sinal de igual)
			return argv[i] + len + 1;
		}
	}
	return 0;
...
int pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc, const char **argv) {
	...
	if (pam_converse(pamh, TOKEN_MESSAGE, &recv_token, PAM_PROMPT_ECHO_ON) != 0) {
		free(token);
		return PAM_CONV_ERR;
	}
